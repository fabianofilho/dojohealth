...
df_path = "../data/df.clean.csv"
wheight_col = 67
clumns_to_remove = [wheight_col]
batch_size = 1
...
wheight = get_colum(lines,wheight_col)
...
result_randomization = get_random_data(cleaned_list,wheight,0.8)
...
train_label = np.array(result_randomization["l2"][0])/100.0#np.array(wheight)
...
eval_label = np.array(result_randomization["l2"][1])/100.0
...
if(METHOD == METHODS[0]):
	...
	syn1 = 3*np.random.random((data_train_len,64)) -2
	syn2 = 4*np.random.random((64,32)) -3
	syn3 = 5*np.random.random((32,16)) -4
	...
	lx_error = eval_label[0] - lx
	print("ERROR:" + str(np.mean(np.abs(lx_error))))
	print("PREDICTED: "+str(lx[0]))
	print("REAL: "+ str(eval_label[0]))
...
if(METHOD == METHODS[1]):
	...
	fully_connected_layer_2 = mx.sym.FullyConnected(data=act1, name='fc2', num_hidden = 64)
